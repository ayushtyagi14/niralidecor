...
const Founder = () => {
        ...
        const fetchFounder = async () => {
            try {
                ...
                if (data) {
                    ...
                    if (founder) setMediaUrl(founder.mediaUrl);
                } else {
    ...
    const uploadFile = async () => {
        ...
        formData.append('path', 'assets/about-us/founder');
    ...
    const deleteImage = async () => {
        ...
        try {
            ...
            const { data, error } = await Supabase
                ...
                .eq('page', 'about-us');  // Ensure only the relevant entry is deleted
    ...
    return (
        <div className="p-6 space-y-4">
            <h2 className="text-2xl font-bold">Founder&apos;s Image</h2>
            {mediaUrl ? (
                <div>
                    {mediaUrl && (mediaUrl.endsWith('.mp4') || mediaUrl.endsWith('.mov')) ? (
                        <video controls className="rounded-md shadow h-[400px]">
                            <source src={mediaUrl} type="video/mp4" />
                            <source src={mediaUrl} type="video/mov" />
                        </video>
                    ) : (
                        <img src={mediaUrl} alt="Banner" className="rounded-md shadow h-[400px]" />
                    )}
                    <button
